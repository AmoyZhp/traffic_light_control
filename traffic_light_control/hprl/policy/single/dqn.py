...
from hprl.util.typing import Action, SampleBatch, State, Transition, TransitionTuple
...
class DQNNew(SingleAgentPolicy):
    def __init__(
        self,
        acting_net: nn.Module,
        target_net: nn.Module,
    ...
    def learn_on_batch(self, sample_batch: SampleBatch) -> List:
        ...
        loss = self.loss_func(q_vals, expected_q_vals)
        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()
        self.update_count += 1
